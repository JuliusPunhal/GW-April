...
#include "April/Config/Module/ChatFilter.Config.hpp"

namespace config = April::ChatFilterConfig;
...
namespace {
	...
	constexpr bool should_ignore( wchar_t const* message )
	{
	...
	void on_message( GW::HookStatus* status, void* )
	{	
		auto& buf = GW::GameContext::instance()->world->message_buff;
		if ( buf.valid() && should_ignore( buf.begin() ) )
		{
			status->blocked = true;
			buf.clear();
		}
	...
	void on_local_message( GW::HookStatus* status, int, wchar_t const* msg )
	{
...
April::ChatFilter::ChatFilter()
{
	using namespace GW::Packet::StoC;
	
	// Callbacks will only be cleaned up during GWCA shutdown.
	GW::StoC::RegisterPacketCallback<MessageServer>( &entry, on_message );
	GW::StoC::RegisterPacketCallback<MessageGlobal>( &entry, on_message );
	GW::StoC::RegisterPacketCallback<MessageLocal>( &entry, on_message );
	GW::Chat::RegisterLocalMessageCallback( &entry, on_local_message );
}
